-- sample1.sql
-- SCOTT.XE (create connetcion : SCOTT/TIGER )
...
-- ROLLBACK; -- transaction시작 이전의 상태로 되돌려라 
COMMIT; -- 영구적으로 DB에 저장하라는 의미
...
-- ------------------------------------------------------
-- 1-2. 복수 행 INSERT 문 (INSERT SELECT문이라고 말함)
-- ------------------------------------------------------
...
AS
SELECT * FROM dept
WHERE 1 = 2;        -- 항상 0을 반환 기존 테이블의 스키마만 복사(데이터 제외)
-- data dictionary (모든 테이블, 모든 컬럼명이 다있는 딕셔너리를 통해 스키마 복사되는것 )
DESC dept;  -- not null은 복사 안됌
DESC mydept;
